...
export const MistakesReviewModal: React.FC<MistakesReviewModalProps> = ({
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {/* Header */}
      <div className="p-6 sm:p-8 rounded-3xl bg-[#161A23] border border-rose-500/30 shadow-2xl mb-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-400 font-extrabold text-xs uppercase tracking-wider mb-2">
        ...
        {mistakes.length > 0 && (
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <button
              ...
              className="p-3 rounded-2xl border border-slate-800 bg-[#0F1218] text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 transition-colors"
      ...
      {mistakes.length === 0 ? (
        <div className="p-12 text-center bg-[#161A23] rounded-3xl border border-slate-800 shadow-xl">
          <div className="w-16 h-16 rounded-full bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center justify-center text-3xl mx-auto mb-3">
            ✨
          </div>
          <h3 className="text-lg font-black text-white">No Mistakes in Queue!</h3>
        ...
        <div className="space-y-3">
          {missedQuestions.map((q) => {
            ...
            return (
              <div
                key={`mistake-${q.id}`}
                className="p-5 rounded-2xl bg-[#161A23] border border-slate-800 shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4"
              >
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-xs font-bold text-emerald-400 bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-1 rounded-lg inline-block">
