...
export const GlossaryModal: React.FC<GlossaryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 dark:bg-[#0A0C10]/80 backdrop-blur-md flex items-center justify-center p-4">
      <div className="bg-white dark:bg-[#161A23] rounded-3xl max-w-2xl w-full p-6 shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col max-h-[85vh] animate-in fade-in zoom-in-95 duration-150 transition-colors">
        ...
        <div className="my-4 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-50 dark:bg-[#0F1218] border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 font-medium placeholder-slate-400 dark:placeholder-slate-500 transition-colors"
          />
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-1 space-y-2.5 scrollbar-thin">
          {entries.length === 0 ? (
            ...
            entries.map(([ru, en]) => (
              <div
                key={ru}
                className="p-3.5 rounded-2xl bg-slate-50 dark:bg-[#0F1218] border border-slate-200 dark:border-slate-800 hover:border-emerald-500/50 hover:bg-emerald-500/5 transition-all"
              >
                <div className="font-extrabold text-slate-900 dark:text-white text-sm">
                  {ru}
                </div>
                <div className="text-xs text-slate-600 dark:text-slate-400 font-medium mt-1 leading-relaxed">
                  {en}
                </div>
              </div>
            ))
        ...
        <div className="mt-4 pt-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
          ...
          <button
            onClick={onClose}
            className="px-4 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-white font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors"
          >
            Close
          </button>
        </div>
